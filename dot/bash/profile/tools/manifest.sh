source ~/dotfiles/dot/bash/profile/tools/asdf.sh
source ~/dotfiles/dot/bash/profile/tools/disk.sh
source ~/dotfiles/dot/bash/profile/tools/docker.sh
source ~/dotfiles/dot/bash/profile/tools/git.sh
source ~/dotfiles/dot/bash/profile/tools/gnu_coreutils.sh
source ~/dotfiles/dot/bash/profile/tools/networking.sh
source ~/dotfiles/dot/bash/profile/tools/permissions.sh
source ~/dotfiles/dot/bash/profile/tools/printing.sh
source ~/dotfiles/dot/bash/profile/tools/tar.sh
source ~/dotfiles/dot/bash/profile/tools/time.sh

source ~/dotfiles/dot/bash/profile/tools/custom/manifest.sh
